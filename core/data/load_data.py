...
class DataSet(Data.Dataset):
    def __init__(self, __C):
        ...
        if self.__C.PRELOAD:
            ...
            self.iid_to_img_feat, self.iid_to_img_bbox, self.iid_to_image_w, self.iid_to_image_h = img_feat_load(self.img_feat_path_list)
            time_end = time.time()
    ...
    def __getitem__(self, idx):
        ...
        if self.__C.RUN_MODE in ['train']:
            ...
            if self.__C.PRELOAD:
                ...
                img_feat_bbox = self.iid_to_img_bbox[str(ans['image_id'])]
                img_feat_image_w = self.iid_to_image_w[str(ans['image_id'])]
                img_feat_image_h = self.iid_to_image_h[str(ans['image_id'])]
            else:
                img_feat = np.load(self.iid_to_img_feat_path[str(ans['image_id'])])
                img_feat_x = img_feat['x'].transpose((1, 0))
                img_feat_bbox = img_feat['bbox']
                img_feat_image_w = img_feat['image_w']
                img_feat_image_h = img_feat['image_h']
            img_feat_iter = proc_img_feat(img_feat_x, self.__C.IMG_FEAT_PAD_SIZE)
            tmp = self.proc(boxes=img_feat_bbox, image_w=img_feat_image_w, image_h=img_feat_image_h)
            img_bbox_iter = proc_img_feat(tmp, self.__C.IMG_FEAT_PAD_SIZE)
            ...
            if self.__C.PRELOAD:
                ...
                img_feat_bbox = self.iid_to_img_bbox[str(ques['image_id'])]
                img_feat_image_w = self.iid_to_image_w[str(ques['image_id'])]
                img_feat_image_h = self.iid_to_image_h[str(ques['image_id'])]
            else:
                img_feat = np.load(self.iid_to_img_feat_path[str(ques['image_id'])])
                img_feat_x = img_feat['x'].transpose((1, 0))
                img_feat_bbox = img_feat['bbox']
        ...
        return torch.from_numpy(img_feat_iter), \
               ...
               torch.from_numpy(ans_iter), torch.from_numpy(img_bbox_iter)


    def __len__(self):
        return self.data_size

    def proc(self, boxes, image_w, image_h):
